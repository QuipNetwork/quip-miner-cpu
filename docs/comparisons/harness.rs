...
fn main() {
    ...
    println!("| kernel | nodes | edges | cores | reads | sweeps | best energy (30 samples) | mean energy | mean time/sample | models/s/core | load |");
    println!("|--------|------:|------:|------:|------:|-------:|-------------------------:|------------:|-----------------:|--------------:|-----:|");
    ...
    for f in factors {
        ...
        for k in KERNELS {
            ...
            // Throughput one core delivers, so a kernel gains nothing by
            // spending more cores for the same wall-clock.
            let per_core = 1000.0 / (mean_ms * cores as f64);
            println!(
                "| {} | {nodes} | {} | {cores} | {READS} | {SWEEPS} | {best} | {mean_energy:.0} | \
                 {mean_ms:.1} ms | {per_core:.3} | {load:.1} |",
                k.name(),
                edges.len()
            );
