...
KERNELS = ["cpu-sa", "cpu-gibbs", "cpu-sb", "cpu-bsb", "cpu-mps", "cpu-mfa"]
# cpu-mps and cpu-mfa share a hue on purpose: select_chi returns 1 on every
# instance here, so they are the same algorithm and their lines coincide.
COLOR = {"cpu-sa": "#555555", "cpu-gibbs": "#aaaaaa", "cpu-sb": "#1f6fb4",
         "cpu-bsb": "#c1521d", "cpu-mps": "#4a7c3f", "cpu-mfa": "#8fb082"}
WIDTH = {"cpu-sa": 1.6, "cpu-gibbs": 1.6, "cpu-sb": 2.4, "cpu-bsb": 2.4,
         "cpu-mps": 2.4, "cpu-mfa": 1.6}
PIVOT = 4577
...
rel, err = {}, {}
for k in ("cpu-gibbs", "cpu-sb", "cpu-bsb", "cpu-mps", "cpu-mfa"):
    means, ses = [], []
    for n in sizes:
...
# Throughput spans 0.024 to 5.2 models per second per core, a factor of 217.
# A linear axis would press the two tensor-network series flat against zero and
# hide the factor of ten between them and the annealing kernels. Throughput is
# ratio data, so a log axis reads correctly: equal vertical distance is equal
# ratio.
A_LO, A_HI = math.log10(0.02), math.log10(6.0)
def say(v):
    return A_BOT - (math.log10(v) - A_LO) / (A_HI - A_LO) * PH
...
add(f'<text x="{ML}" y="76" font-size="11" fill="#666">'
    'cpu-mps and cpu-mfa return identical energies here: the bond budget resolves to 1 on every instance, so both run as a product state.</text>')
...
add(f'<line x1="{ML}" y1="{say(0.02)}" x2="{ML}" y2="{say(6.0)}" stroke="#111" stroke-width="1"/>')
for v in (0.03, 0.1, 0.3, 1.0, 3.0):
    add(f'<text x="{ML - 8}" y="{say(v) + 3.5:.1f}" font-size="10" fill="#444" '
        f'text-anchor="end">{v:g}</text>')
add(f'<text x="{ML - 8}" y="{say(6.0) - 4:.1f}" font-size="10" fill="#444" '
    'text-anchor="end">per core</text>')
add(f'<text x="{ML - 8}" y="{say(6.0) + 8:.1f}" font-size="9" fill="#999" '
    'text-anchor="end">log scale</text>')
...
off_lo = min(rel["cpu-mps"])
off_hi = max(rel["cpu-mps"])
add(f'<text x="{sx(x1) + 10}" y="{B_TOP - 15}" font-size="10" fill="{COLOR["cpu-mps"]}">'
    f'cpu-mps and cpu-mfa: +{off_lo:.1f} to +{off_hi:.1f}%, off scale</text>')
...
# cpu-mps and cpu-mfa sit at +13.8 to +15.7 percent, thirty times the range of
# every other series. Drawing them here would flatten the contest this panel
# exists to show, so the strip above states their range instead.
for k in ("cpu-gibbs", "cpu-sb", "cpu-bsb"):
